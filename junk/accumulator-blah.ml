...
let stepper (module SM : STATEMACHINE) =
  let state = SM.get_start_state () in
  { state }

let step (type s) (type i) (type o)
    (module SM : STATEMACHINE
      with type state = s
       and type input = i
       and type output = o) stepper =
 fun input ->
  let s, o = SM.get_next_state ~state:stepper.state ~input in
  stepper.state <- s;
  o
...
let trace (type s) (type i) (type o)
  ...
  |> List.mapi (fun i input ->
      let o = step (module SM) stepper input in
...
let () =
  let accumulator = make_accumulator ~initial_value:0 in
